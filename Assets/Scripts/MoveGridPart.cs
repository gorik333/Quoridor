using System.Collections;
using System.Collections.Generic;
using UnityEngine;


public enum AvailableDirection
{
    Right,
    Left,
    Top,
    Down
}


public class MoveGridPart : MonoBehaviour
{
    #region Serialize data

    [SerializeField]
    private Vector2Int _gridPos;

    [SerializeField]
    private SpriteRenderer _spriteRenderer;

    [SerializeField]
    private Sprite _allowSprite;

    [SerializeField]
    private Transform _pawnMovePos;

    #endregion

    private bool _isAllowMoveTo;
    private bool _isWithPawn;

    private List<AvailableDirection> _availableDirection;

    public event Move OnMoveTo;
    public delegate void Move(MoveGridPart moveGridPart);


    private void OnEnable()
    {
        _availableDirection = new List<AvailableDirection>();

        _availableDirection.Add(AvailableDirection.Right);
        _availableDirection.Add(AvailableDirection.Left);
        _availableDirection.Add(AvailableDirection.Top);
        _availableDirection.Add(AvailableDirection.Down);
    }


    public void UpdateState(bool isAllowMoveTo)
    {
        _isAllowMoveTo = isAllowMoveTo;

        _spriteRenderer.gameObject.SetActive(isAllowMoveTo);
    }


    public bool MoveTo()
    {
        if (_isAllowMoveTo)
            OnMoveTo(this);

        return _isAllowMoveTo;
    }


    public Vector2Int GridPos { get => _gridPos; }

    public Vector3 PawnMovePos { get => _pawnMovePos.position; }

    public bool IsWithPawn { get => _isWithPawn; }
}
